...
rint_min = float('inf')
rint_max = 0.0
for rint_collection in (bb_rint_start, bb_rint_end):
    for k, v in rint_collection.items():
        rint = statistics.mean(v)
        rint_min = min(rint_min, rint)
        rint_max = max(rint_max, rint)

rint_min_beak = ((1000 * rint_min) - 1) / 1000
rint_max_beak = ((1000 * rint_max) + 1) / 1000

rint_min = float('inf')
rint_max = 0.0
for rint_collection in (rint_reverse_start, rint_reverse_end):
    for k, v in rint_collection.items():
        rint = statistics.mean(v)
        rint_min = min(rint_min, rint)
        rint_max = max(rint_max, rint)

rint_min_tester = ((1000 * rint_min) - 1) / 1000
rint_max_tester = ((1000 * rint_max) + 1) / 1000

mpl.rcParams["savefig.directory"] = "."
mpl.rcParams["savefig.format"] = "pdf"

fig, ax = plt.subplots(2, 2, layout="constrained")
one_scatter_plot(ax[0, 1], ce, rint_reverse_start, "$r_{int}$ @ start (tester)", rint_min_tester, rint_max_tester)
one_scatter_plot(ax[1, 1], ce, rint_reverse_end, "$r_{int}$ @ end (tester)", rint_min_tester, rint_max_tester)
one_scatter_plot(ax[0, 0], ce, bb_rint_start, "$r_{int}$ @ start (beak)", rint_min_beak, rint_max_beak)
one_scatter_plot(ax[1, 0], ce, bb_rint_end, "$r_{int}$ @ end (beak)", rint_min_beak, rint_max_beak)

fig.suptitle("$r_{int}$ vs Capacity", fontweight='bold')
